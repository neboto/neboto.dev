...
- **Browse the data, not just the config.** <kbd>o</kbd> walks a bucket like a file manager, with version history, previews and downloads; <kbd>i</kbd> scans or queries a DynamoDB table. Open a `.tfstate` from the bucket and <kbd>Enter</kbd> jumps from any Terraform address to the live resource.
